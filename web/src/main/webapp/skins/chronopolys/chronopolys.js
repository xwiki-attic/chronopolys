...
function toggleContainerAdd() {
    if (elementsAddCache == null) {
        elementsAddCache = $('chronoapp').select('.container_add');
    }
...
function switchDashboardMode(mode)
{
    ...
    if (elementsCache == null) {
        elementsCache = $('chronoapp').select('.mode');
    }
    // save desired mode
    currentMode = mode;
...
{
    regDatesList.clear();
    var regDatesElts = $('projectphases_edit').select('.registereddate');
...
function getContainersStates()
{
    if (visibleEls == null || visibleEls.length == 0) {
        visibleEls = $('axis_all').select('.hiddenable');
    }
    var elist = new Array();
    var str = '[';
  ...
  makeKeyDownHandler: function(self)
  {
    return function(ev) {
      ...
      { 
        ...
        self.updHandler(self.slideNo, self.dp);
      }
      ...
      if((code == 37 || code == 39) && self.keyDown && self.temp != self.dp) {
          ...
          new Ajax.Request(url, {
              ...
              onFailure: function(transport) {
                ...
                pivot.updHandler(pivot.slideNo, pivot.tempdp);
              }
          });
      }
      self.keyDown = false;
