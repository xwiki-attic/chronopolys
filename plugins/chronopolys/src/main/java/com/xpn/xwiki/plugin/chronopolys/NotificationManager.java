    ...
    implements XWikiDocChangeNotificationInterface, XWikiActionNotificationInterface
{
    ...
    /* public static final String NOTIFICATION_MEETING_INVITATION =
        NOTIFICATION_SPACE + "." + "MeetingInvitation" + NOTIFICATION_SUFFIX; */
                ...
                if (project != null) {
                    ...
                    if (project.get("status").equals("1")) {
                        ...
                        if (newdoc != null && olddoc != null &&
                            Project.PROJECT_HOMEDOC.equals(newdoc.getName())
                            ...
                            .equals(olddoc.getStringValue("status")))
                        {
                            ...
                            if (newdoc.getStringValue("status").equals("1")) {
                                ...
                                WatchListPlugin wlplugin = (WatchListPlugin) context.getWiki()
                                    .getPlugin("watchlist", context);
                                Iterator it = rcpt.iterator();
                                while (it.hasNext()) {
                                    wlplugin.addWatchedElement((String) it.next(),
                                        project.getSpace(), true, context);
                                }
                            }
                            ...
                            if (project.isPlogTask(newdoc)) {
                                if (olddoc == null ||
                                    (olddoc != null && !newdoc.getStringValue("taskassignee")
                        ...
                        if (rcpt.size() > 0 && !mailTemplate.equals("")) {
                            if (!context.getWiki().exists(mailTemplate, context)) {
                                LOG.error("Mail template : " + mailTemplate + "does not exist");
                                return;
                            }
                            /* Send notifications */
                            this.sendNotification(rcpt, project, new Document(newdoc, context),
